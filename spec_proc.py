...
def spec_proc(data_tab):
    ...
    for i in range(len(s)):
        print i
        z= d['z'][i]
        Av= 3.1 * s['EXTINCTION'][i][1] #use the extinction in the g magnitude E(B-V)
        spec_name= './data/spec-'+str(s['palte'][i])+'-'+str(s['MJD_spec'][i])+'-'+str(s['fiberid'][i]).zfill(4)+'.fits' # fits file name
